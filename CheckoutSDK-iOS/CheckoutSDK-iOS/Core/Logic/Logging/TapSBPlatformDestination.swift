...
internal class SBPlatformDestination: BaseDestination {
    ...
    struct SendingPoints {
        var verbose = 0
        var debug = 1
        var info = 5
        var warning = 8
        var error = 10
        var threshold = 10  // send to server if points reach that value
    }
    var sendingPoints = SendingPoints()
    var showNSLog = false // executes toNSLog statements to debug the class
    var points = 0

    var serverURL = URL(string: "https://api.swiftybeaver.com/api/entries/") // optional
    var entriesFileURL = URL(fileURLWithPath: "") // not optional
    var sendingFileURL = URL(fileURLWithPath: "")
    var analyticsFileURL = URL(fileURLWithPath: "")
