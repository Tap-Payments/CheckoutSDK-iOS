...
internal class BaseDestination: Hashable, Equatable {

    /// output format pattern, see documentation for syntax
    internal  var format = "$DHH:mm:ss.SSS$d $C$L$c $N.$F:$l - $M"

    /// runs in own serial background thread for better performance
    internal  var asynchronously = true

    /// do not log any message which has a lower level than this one
    internal  var minLevel = SwiftyBeaver.Level.verbose

    /// set custom log level words for each level
    internal  var levelString = LevelString()

    /// set custom log level colors for each level
    internal  var levelColor = LevelColor()

    internal  struct LevelString {
        var verbose = "VERBOSE"
        var debug = "DEBUG"
        var info = "INFO"
        var warning = "WARNING"
    ...
    func send(_ level: SwiftyBeaver.Level, msg: String, thread: String, file: String,
...
func == (lhs: BaseDestination, rhs: BaseDestination) -> Bool {
